...
Pulse publishes per-peer assignment changes **directly to NATS**: `peer.{addr}.cluster_change`, carrying `decentraland.pulse.PeerClusterChange { cluster_id, realm, session, displaced_session, displaced_cluster_id }` from `decentraland/pulse/pulse_clusters.proto`, emitted whenever a peer's *published* (post-debounce) assignment changes. The wallet is lower-cased so one wallet always maps to one subject regardless of the checksum casing the auth chain carried.

**One wallet, one publishing peer.** A subject is per wallet, but a wallet can briefly own two `PeerIndex`es: `EvictDuplicateSession` disconnects the incumbent asynchronously and cannot flip its state — it belongs to another worker shard — so it stays in the grid until its transport disconnect lands, up to `Transport:PeerTimeoutMs`. The tracker therefore collects only the peer `IdentityBoard` currently binds the wallet to. Without that, both peers publish onto one subject, the outbox coalesces them latest-wins in grid-cell order, and the incoming session can be left addressed by the outgoing one's cluster with no further event to correct it.

**Session key and takeover.** Every event carries `session`: the lower-cased ephemeral address of the auth chain the peer authenticated with (the wallet itself for a chain without a delegation), read from `IdentityBoard`. A wallet's last published assignment is retained per wallet for `Clusters:SessionRetentionPasses` passes (default 300; `0` disables). The first publish of a *new* session for a wallet whose retained assignment belonged to another session carries `displaced_session` and `displaced_cluster_id` — that other session and the cluster it was last published into. A same-session reconnect carries neither. The assignment itself is always the peer's own cluster; nothing is steered into the displaced session's room. Counted by `dcl_pulse_cluster_takeovers_total`.

Ending the displaced session is the consumer's job: comms-gatekeeper removes `displaced_session`'s participant from `island-{displaced_cluster_id}` with a token revocation before it mints the new session's token, and publishes the connection string on a session-addressed subject that WS Connector forwards only to the socket holding that session. No client change is involved.
...
Delivery semantics remain at-most-once, so a dropped event still leaves a peer in its previous room until its next reassignment. The outbox makes that far less likely — an outage now retains each peer's latest assignment rather than discarding it — and the design's original mitigation, a low-rate periodic re-publish sweep, is implemented: with `Clusters:RepublishIntervalPasses` above zero a peer's settled assignment is re-emitted on `peer.{addr}.cluster_refresh` once it has gone that many passes without reaching the feed. The separate subject is what lets a consumer tell a refresh from a real assignment event — a refresh is safe to ignore when the consumer already holds the assignment, a change never is. Ships at zero, i.e. off. `GET /islands` (once it exists, §3.5) remains the bulk reconciliation path.
...
| `SessionRetentionPasses` | 300 | Passes a departed wallet's last published assignment is retained so a new session's first publish can name it as displaced (§3.6); `0` disables |
...
Everything else archipelago does is already Pulse-native and needs no migration: AuthChain auth, duplicate-session eviction (plus the session key and takeover annotation above, which archipelago had no equivalent of), heartbeat expiry (Pulse: ~5 s disconnect cleanup vs 60 s), and kick messaging (`DisconnectReason`).
...
- The periodic re-publish sweep (§3.6) is implemented but ships disabled (`Clusters:RepublishIntervalPasses: 0`). Enabling it requires comms-gatekeeper to suppress a refresh for an assignment it already holds: it mints a LiveKit token per event with no no-op suppression, and the explorer treats any new `connStr` as a room change, so an unsuppressed sweep would make every peer reconnect into the room it already occupies. Pick the interval against `Nats:ChannelCapacity` and that per-event consumer cost.
