    ...
    private void UpdateBot(BotSession bot)
    {
        ...
        bot.Pipe.Send(new MessagePipe.OutgoingMessage(new ClientMessage
        {
            Input = new PlayerStateInput
            {
                ...
                {
                    HeadPitchQuantized = 0f,
                    HeadYawQuantized = rotationY,
                    GlideState = new GlideState(),
                    MovementBlendQuantized = moving ? 1f : 0f,
                    PositionXQuantized = relativePosition.X,
                    PositionYQuantized = relativePosition.Y,
                    PositionZQuantized = relativePosition.Z,
                    RotationYQuantized = rotationY,
                    SlideBlendQuantized = 0f,
            ...
            bot.Pipe.Send(new MessagePipe.OutgoingMessage(new ClientMessage
            {
                EmoteStart = new EmoteStart
                {
                    ...
                    PlayerState = new PlayerState
                    {
                        ...
                        JumpCount = bot.JumpCount,
                        ParcelIndex = parcelIndex,
                        VelocityXQuantized = 0f,
                        VelocityYQuantized = 0f,
                        VelocityZQuantized = 0f,
                    },
                },
            }, PacketMode.RELIABLE));
        }
