...
| **Assignment requests throttled rising** | The responder is shedding a request flood. Legitimate lookups in that second timed out and retry on the next hint; the lever is `Nats:MaxAssignmentRequestsPerSecond` or broker permissions on `peer.*` |
| `Dropped N cluster assignment requests over the L/s budget` (Warning, at most once per second) | The same refusals, summed per window. Emitted when the next window opens, so a flood's final window is logged late or, if no request follows, never; read the counter for timing |
| **Assignment requests rejected rising** | A publisher other than gatekeeper is sending requests without a request inbox. Nothing is answered and the budget is untouched; tighten broker permissions on `peer.*` |
...
How far a client's baseline had fallen behind when it asked for a resync: `latestSnapshot.Seq − knownSeq` in **snapshot sequence numbers**, recorded where the simulation answers the request off the snapshot ring — one sample per subject per tick. This replaced a per-request log line — the distribution answers the same question the log did (how big are the gaps clients recover from?) without one line per event.

A request that a discrete event supersedes in the same tick is **not** sampled: `PlayerJoined`, `Teleported` and `EmoteStarted` each carry full state, consume the pending request and skip the resync path entirely. Nothing is lost — the client gets its full state from a message that was going out anyway, so the resync cost the server nothing extra — but it means the sample count trails the served-resync rate, and the shortfall is all full-state answers. Read the `full` share as a floor, most visibly during a teleport or emote storm, which is exactly when the two coincide.
