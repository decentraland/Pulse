...
internal static class PrometheusFormatter
{
    ...
    [
        ...
        ClientMessage.MessageOneofCase.SceneListenerUpdate,
    ];
    ...
    public static void Write(StreamWriter writer, MetricsSnapshot snap)
    {
        ...
        WriteCounter(writer, "dcl_pulse_cluster_takeovers_total", "First publishes of a new session for a wallet whose retained assignment belonged to another session. Each one is a wallet taken over by a second device.", snap.Clusters.TotalTakeovers);
        ...
        WriteCounter(writer, "dcl_pulse_nats_publish_failed_total", "Feed, heartbeat, assignment reply and recovery hint publishes that threw. Raised client-side — timeout, connect failure, oversized payload, rejected subject — since a PUB is never acknowledged. Non-zero means the broker or the path to it needs attention, not more outbox capacity.", snap.Clusters.TotalNatsPublishFailed);
        ...
        WriteCounter(writer, "dcl_pulse_nats_assignment_requests_rejected_total", "cluster_assignment requests discarded because their reply subject is missing or not a request inbox. They spend no request budget; non-zero means something other than gatekeeper publishes on peer.*.", snap.Clusters.TotalNatsAssignmentRequestsRejected);
        WriteCounter(writer, "dcl_pulse_nats_assignment_requests_throttled_total", "cluster_assignment requests refused over Nats:MaxAssignmentRequestsPerSecond, counted as they are refused.", snap.Clusters.TotalNatsAssignmentRequestsThrottled);
