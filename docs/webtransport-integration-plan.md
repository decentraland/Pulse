...
**Fix (zero protocol change):** the C# channel-semantics layer frames each datagram with a small header `{ channelId : u8, seq : u16/u32 }`. Outbound it assigns the next seq per `(peer, channelId)`; inbound, for `UNRELIABLE_SEQUENCED` it tracks the highest seq seen per `(peer, channelId)` and **drops older** ones before handing bytes up — replicating ENet's behaviour. `UNRELIABLE_UNSEQUENCED` skips the dedup. This keeps `.proto` and all handlers untouched and matches Pulse's "channel semantics by convention" philosophy. Handle seq wraparound.

### Datagram size cap (~1200 B path-MTU)
ENet fragments large unreliable packets; WT datagrams cannot exceed MTU (browsers enforce strictly). Audit the largest unreliable payloads (`PlayerStateDeltaTier0`, `PlayerStateInput` — both small/quantized, expected to fit). Add a guard: if a datagram-bound message exceeds the cap, **fall back to the reliable stream** (or drop + count). New metric for oversize events. **Site this guard in the WT channel-semantics layer, never as a branch inside `PeerSimulation`** — the simulation must stay transport-agnostic (CLAUDE.md `PeerSimulation` decoupling rule).
...
- **New WT-specific metrics** (use the `add-metric` skill end-to-end — instrument → accumulate → snapshot → Prometheus → dashboard): `active_streams`, `datagrams_dropped_stale`, `datagrams_dropped_oversize`, `datagram_bytes` vs `stream_bytes`, `quic_rtt` (gauge), `tls_handshake_failures`, `send_queue_overflow_disconnects`, `cert_expiry_days` (gauge — operationally important).
...
- **End-to-end**: run the server with both transports enabled; connect a **real browser** WebTransport client (or a Deno/Node WebTransport script) using a MetaForge-issued ECDSA chain → complete handshake → send MovementInput → observe STATE_DELTA. Confirm the console dashboard shows `webtransport` peers and Prometheus emits `transport="webtransport"`. In parallel, connect a `DCLPulseTestClient` (ENet) and confirm both coexist and observe each other.
- **Note**: `DCLPulseTestClient` is ENet-only today; a browser/Deno WT smoke client (reusing the MetaForge auth chain) is part of this work.
