...
| `HandoverPasses` | 15 | Passes a departed wallet's last published assignment is retained for the session handover (§3.6); `0` disables |
...
Everything else archipelago does is already Pulse-native and needs no migration: AuthChain auth, duplicate-session eviction (plus the cluster-feed handover above, which archipelago had no equivalent of), heartbeat expiry (Pulse: ~5 s disconnect cleanup vs 60 s), and kick messaging (`DisconnectReason`).
