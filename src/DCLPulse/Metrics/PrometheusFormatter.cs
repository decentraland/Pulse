...
internal static class PrometheusFormatter
{
    ...
    public static void Write(StreamWriter writer, MetricsSnapshot snap)
    {
        ...
        WriteCounter(writer, "dcl_pulse_cluster_handovers_total", "Incoming duplicate sessions published into an outgoing session's cluster, so LiveKit supersedes the outgoing participant. Each one is a wallet that was connected twice.", snap.Clusters.TotalHandovers);
