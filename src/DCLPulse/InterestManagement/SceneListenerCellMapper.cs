    ...
    IOptions<ParcelEncoderOptions> parcelOptions)
{
    ...
    public long[] ComputeCellKeys(IReadOnlyCollection<int> parcelIndices)
    {
        ...
        foreach (int index in parcelIndices)
        {
            ...
            keys.Add(realmGrids.ComputeCellKey(minX, minZ));
            keys.Add(realmGrids.ComputeCellKey(minX + parcelSize, minZ));
            keys.Add(realmGrids.ComputeCellKey(minX, minZ + parcelSize));
            keys.Add(realmGrids.ComputeCellKey(minX + parcelSize, minZ + parcelSize));
        }
