...
**Realm partitioning.** Each realm has its own `SpatialGrid`, held by `RealmSpatialGrids`; `PeerSnapshot.Realm` decides which one a peer is written into. Visibility is therefore partitioned by construction — an observer resolves its own realm's grid and every candidate it finds is already same-realm, so collection never compares realms; the simulation only re-checks each subject's latest snapshot against the observer's AoI (its realm, or for a scene listener an announced parcel of that realm), to reject one that teleported after collection. A peer with no realm yet occupies no grid at all: invisible to everyone, and it sees nobody. A cross-realm teleport vacates the old grid *before* publishing the snapshot, so the peer is briefly in no grid rather than sitting in the old realm's grid under a snapshot that already names the new one — the latter would surface to observers of the old realm as a cross-realm subject and stick until the stale-view sweep. A single Pulse instance transparently hosts multiple realms that never see each other. The subject's realm rides along on `PlayerJoined`, so the client always knows which realm a peer belongs to. A realm change is never sent as `Teleported`: a subject that changes realm is retired with `PlayerLeft` (and re-announced with a `PlayerJoined` carrying the new realm to a scene listener that announced its new parcel), and an observer that changes realm has all its views retired and reseeded.
